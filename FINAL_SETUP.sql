-- Run this ONCE in Supabase SQL Editor, then I'll auto-deploy everything!

CREATE OR REPLACE FUNCTION create_battery_storage_table()
RETURNS void AS $$
BEGIN
  ...
  DROP POLICY IF EXISTS "Public read" ON battery_storage;
  CREATE POLICY "Public read" ON battery_storage FOR SELECT USING (true);
  DROP POLICY IF EXISTS "Allow all" ON battery_storage;
  CREATE POLICY "Allow all" ON battery_storage FOR ALL USING (true) WITH CHECK (true);
END;
$$ LANGUAGE plpgsql;

SELECT create_battery_storage_table();
